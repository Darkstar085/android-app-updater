...
def build_release_notes(data: dict) -> str:
    ...
    if data["failed"]:
        ...
        notes.extend(
            f"- **{name}** — {reason}"
            for name, reason in data["failed"]
        )
...
def main() -> None:
    ...
    release_notes = build_release_notes(data)
    update_summary = build_update_summary(updated)
    write_github_environment(release_notes, update_summary)
